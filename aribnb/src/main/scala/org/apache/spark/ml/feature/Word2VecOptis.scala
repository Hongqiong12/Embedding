...
/**
 * Params for [[Word2VecOpti]] and [[Word2VecOptiModel]].
 */
private[feature] trait Word2VecOptiBase extends Params
  ...
  /**
   * The minimum number of times a token must appear to be included in the Word2VecOpti model's
   * vocabulary.
   * Default: 5
   * @group param
   */
  final val minCount = new IntParam(this, "minCount", "the minimum number of times a token must " +
    "appear to be included in the Word2VecOpti model's vocabulary (>= 0)", ParamValidators.gtEq(0))
...
/**
 * Word2VecOpti trains a model of `Map(String, Vector)`, i.e. transforms a word into a code for further
 * natural language processing or machine learning process.
 */
@Since("1.4.0")
final class Word2VecOptis @Since("1.4.0") (
  ...
  @Since("2.0.0")
  override def fit(dataset: Dataset[_]): Word2VecOptiModel = {
    transformSchema(dataset.schema, logging = true)
    val input = dataset.select($(inputCol)).rdd.map(_.getAs[Seq[String]](0))
    val wordVectors = new feature.Word2VecOpti()
  ...
  @Since("1.4.1")
  override def copy(extra: ParamMap): Word2VecOptis = defaultCopy(extra)
}

@Since("1.6.0")
object Word2VecOpti extends DefaultParamsReadable[Word2VecOptis] {

  @Since("1.6.0")
  override def load(path: String): Word2VecOptis = super.load(path)
}

/**
 * Model fitted by [[Word2VecOpti]].
 */
@Since("1.4.0")
class Word2VecOptiModel private[ml] (
                                      @Since("1.4.0") override val uid: String,
                                      @transient private val wordVectors: feature.Word2VecOptiModel)
  extends Model[Word2VecOptiModel] with Word2VecOptiBase with MLWritable {

  import Word2VecOptiModel._
  ...
  override def transform(dataset: Dataset[_]): DataFrame = {
    ...
    val d = $(vectorSize)
    val Word2VecOpti = udf { sentence: Seq[String] =>
      if (sentence.isEmpty) {
    ...
    dataset.withColumn($(outputCol), Word2VecOpti(col($(inputCol))))
  }
  ...
  @Since("1.4.1")
  override def copy(extra: ParamMap): Word2VecOptiModel = {
    val copied = new Word2VecOptiModel(uid, wordVectors)
    copyValues(copied, extra).setParent(parent)
  }

  @Since("1.6.0")
  override def write: MLWriter = new Word2VecOptiModelWriter(this)
}

@Since("1.6.0")
object Word2VecOptiModel extends MLReadable[Word2VecOptiModel] {

  private case class Data(word: String, vector: Array[Float])

  private[Word2VecOptiModel]
  class Word2VecOptiModelWriter(instance: Word2VecOptiModel) extends MLWriter {
    ...
    override protected def saveImpl(path: String): Unit = {
      ...
      val numPartitions = Word2VecOptiModelWriter.calculateNumberOfPartitions(
  ...
  private[feature]
  object Word2VecOptiModelWriter {
    /**
    ...
    def calculateNumberOfPartitions(
      ...
      require(numPartitions < 10e8, s"Word2VecOptiModel calculated that it needs $numPartitions " +
        s"partitions to save this model, which is too large.  Try increasing " +
        s"spark.kryoserializer.buffer.max so that Word2VecOptiModel can use fewer partitions.")
      numPartitions.toInt
    }
  }

  private class Word2VecOptiModelReader extends MLReader[Word2VecOptiModel] {

    private val className = classOf[Word2VecOptiModel].getName

    override def load(path: String): Word2VecOptiModel = {
      val spark = sparkSession
      import spark.implicits._
      ...
      val oldModel = if (major < 2 || (major == 2 && minor < 2)) {
        ...
        new feature.Word2VecOptiModel(wordIndex, wordVectors)
      } else {
        val wordVectorsMap = spark.read.parquet(dataPath).as[Data]
          .collect()
          .map(wordVector => (wordVector.word, wordVector.vector))
          .toMap
        new feature.Word2VecOptiModel(wordVectorsMap)
      }

      val model = new Word2VecOptiModel(metadata.uid, oldModel)
  ...
  @Since("1.6.0")
  override def read: MLReader[Word2VecOptiModel] = new Word2VecOptiModelReader

  @Since("1.6.0")
  override def load(path: String): Word2VecOptiModel = super.load(path)
}
